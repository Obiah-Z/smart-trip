...
class ExecutorAgent:
    ...
    def run(
        ...
        *,
        ...
        route_result = tool_result(tool_results, TOOL_NAME_MAP["route"])
        hotel_result = tool_result(tool_results, TOOL_NAME_MAP["hotel"])
        weather_result = tool_result(tool_results, TOOL_NAME_MAP["weather"])
        attraction_result = tool_result(tool_results, TOOL_NAME_MAP["attraction"])
        budget_result = tool_result(tool_results, TOOL_NAME_MAP["budget"])
        ...
        *,
        ...
        for attraction in attractions[:6]:
            ...
            if attraction_type in preferences:
                reasons.append(f"契合你想看的{ATTRACTION_TYPE_LABELS.get(attraction_type, attraction_type)}")
            ...
            recommendations.append(
                {
                    ...
                    "typeLabel": ATTRACTION_TYPE_LABELS.get(attraction_type, attraction_type),
            ...
            guides.append(
                {
                    ...
                    "paceLabel": PACE_LABELS.get(pace, pace),
    ...
    def _build_planning_narrative(
        ...
        *,
        ...
        parts = [f"{destination} 这次会按{PACE_LABELS.get(pace, pace)}节奏来走"]
        if area_names:
