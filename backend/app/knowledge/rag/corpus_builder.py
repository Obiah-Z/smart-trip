...
from app.knowledge.rag.indexer import RAGIndexBuilder
        ...
        chunks.append(
            {
                ...
                "content": content,
                "keywords": keywords,
                    "source": _relative_source_path(file_path),
...
def _relative_source_path(file_path: Path) -> str:
    """Return a stable source path even if the package directory depth changes."""
    try:
        return str(file_path.relative_to(backend_root()))
    except ValueError:
